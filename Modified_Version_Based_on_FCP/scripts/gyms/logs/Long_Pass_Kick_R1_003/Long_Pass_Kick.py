...
class Long_Pass_Kick(gym.Env):
    ...
    def step(self, action):
        ...
        tuned_res = action[1:].astype(np.float32) * 3.0   # ~ +/-3deg on selected joints
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        # Keep it simple and robust: 1 env + DummyVecEnv (no extra subprocess layer)
        n_envs = 1
        n_steps_per_env = 256
        minibatch_size = 64          # must be <= n_steps_per_env * n_envs
        total_steps = 300_000
        learning_rate = 3e-4
        ...
        # Drawings go to Windows RoboViz (WSL -> Windows). Use Run_Utils -d <WIN_IP>
        draw_host = getattr(self.script.args, "d", self.ip)

        def init_env(i_env):
            def thunk():
                return Long_Pass_Kick(
                    self.ip,
                    self.server_p + i_env,
        ...
        try:
            # Start n_envs soccer servers (Gym starts its own servers, do NOT run docker server manually)
            servers = Server(self.server_p, self.monitor_p_1000, n_envs)

            # For a single env, DummyVecEnv is more stable than SubprocVecEnv
            env = DummyVecEnv([init_env(0)])
            ...
            self.learn_model(
                ...
                eval_env=None,
                eval_freq=None,
                save_freq=n_steps_per_env * 50,
                backup_env_file=__file__
            )

        finally:
            # When the simulator crashes, SB3 may throw EOFError during close.
            # Make cleanup best-effort so you can restart quickly.
            try:
                if env is not None:
                    env.close()
            except Exception as e:
                print("[WARN] env.close() failed:", e)

            try:
                if servers is not None:
                    servers.kill()
            except Exception as e:
                print("[WARN] servers.kill() failed:", e)
