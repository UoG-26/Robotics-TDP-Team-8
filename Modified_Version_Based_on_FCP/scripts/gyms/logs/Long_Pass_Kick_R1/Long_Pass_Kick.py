...
class Long_Pass_Kick(gym.Env):
    ...
    def reset(self):
        ...
        # Stabilize + set training-friendly state
        for _ in range(10):
            self.player.scom.unofficial_set_play_mode("PlayOn")
            self.player.scom.unofficial_set_game_time(0)
            self.player.scom.unofficial_beam((*self.robot_beam, 0.50), self.robot_ori)  # float
            self.player.behavior.execute("Zero_Bent_Knees")
            self._sync()

        # Beam on ground
        self.player.scom.unofficial_beam((*self.robot_beam, r.beam_height), self.robot_ori)
        r.joints_target_speed[0] = 0.01  # trigger physics update
        self._sync()

        # place ball in kick area (absolute coords)
    ...
    def step(self, action):
        ...
        angle_res = action[1:].astype(np.float32) * 8.0  # ~ +/-8deg residuals
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        n_steps_per_env = 64
        minibatch_size = 256  # must divide n_envs*n_steps
