...
class Agent(Base_Agent):
    def __init__(self, host:str, agent_port:int, monitor_port:int, unum:int,
                 team_name:str, enable_log, enable_draw, wait_for_server=True, is_fat_proxy=False,draw_host=None) -> None:
        ...
        self.init_pos = ([-14,0],[-9,0],[-9,0],[-9,0],[-5,-5],[-5,0],[-5,5],[-1,-6],[-1,-2.5],[-1,2.5],[-1,6])[unum-1] # initial formation
        # ---- role assignment by uniform number
        self.role = self.get_role(unum)   # "GK" / "DEF" / "STL" / "STR" / "OTHER"

    def get_role(self, unum: int) -> str:
        if unum == 1:
            return "GK"
        elif unum == 2:
            return "DEF"
        elif unum == 9:
            return "STL"  # left striker
        elif unum == 10:
    ...
    def think_and_send(self):
        w = self.world
        if self.enable_draw and not hasattr(self, "_draw_once"):
            from world.commons.Draw import Draw
            print("DRAW peer =", Draw._socket.getpeername())
            print("DRAW prefix =", w.draw._prefix)
            self._draw_once = True
        ...
        if PM == w.M_GAME_OVER:
            ...
            # ---- GK: stay close to goal, track ball Y
            if self.role == "GK":
                gk_x = self.init_pos[0]
                gk_y = float(np.clip(ball_2d[1], -1.0, 1.0))  # follow ball in small range
                self.move((gk_x, gk_y), orientation=ball_dir)

            # ---- DEF: stay behind, between ball and our goal
            elif self.role == "DEF":
                # keep defender deeper than the ball, but not too deep
                def_x = min(self.init_pos[0] + 2.0, ball_2d[0] - 2.0)
                def_x = float(np.clip(def_x, -13.5, -3.0))
                def_y = float(np.clip(ball_2d[1], -4.0, 4.0))
                self.move((def_x, def_y), orientation=ball_dir, priority_unums=[active_player_unum])

            # ---- Strikers: push forward, keep lane separation by init_pos[1]
            elif self.role in ("STL", "STR"):
                st_x = min(ball_2d[0] + 2.5, 13.0)
                st_y = self.init_pos[1]  # keep left/right striker lanes
                self.move((st_x, st_y), orientation=ball_dir, priority_unums=[active_player_unum])

            else:
                # fallback: original formation rule
            ...
            enable_pass_command = (PM == w.M_PLAY_ON and ball_2d[0]<6)
                       ...
                       # ===== role-based active behavior =====

            # GK active: only chase/clear if ball is close to our goal
            if self.role == "GK":
                if ball_2d[0] < -10.0:
                    # clear to midfield
                    clear_dir = M.target_abs_angle(ball_2d, (0.0, 0.0))
                    self.state = 0 if self.kick(clear_dir, 9, False, enable_pass_command) else 2
                else:
                    # return to goal line
                    self.move((self.init_pos[0], float(np.clip(ball_2d[1], -1.0, 1.0))), orientation=ball_dir)

            # DEF active: clear ball to side / midfield, avoid risky shot
            elif self.role == "DEF":
                clear_target = (0.0, float(np.sign(ball_2d[1]) * 6.0))
                clear_dir = M.target_abs_angle(ball_2d, clear_target)
                self.state = 0 if self.kick(clear_dir, 9, False, enable_pass_command) else 2

            # Strikers active: aim to far post
            elif self.role in ("STL", "STR"):
                # left striker aim slightly to right post, right striker aim slightly to left post
                goal_target = (15.05, 0.8) if self.role == "STL" else (15.05, -0.8)
                shot_dir = M.target_abs_angle(ball_2d, goal_target)
                self.state = 0 if self.kick(shot_dir, 9, False, enable_pass_command) else 2
        ...
        if self.enable_draw:
            ...
            if active_player_unum == r.unum:
                ...
                d.annotation(
                    (*my_head_pos_2d, 0.8),
                    f"ACTIVE u{r.unum}  PM={PM}  ball_spd={ball_speed:.2f}",
                    d.Color.yellow,
                    "status",
                    flush=False
                )
                d.line(ball_2d, (15.05, 0), 2, d.Color.green_light, "status", flush=False)
                d.annotation((*my_head_pos_2d, 0.6), f"ACTIVE {self.role}" , d.Color.yellow, "status")

                # 2) 画线：自己 -> 球
                d.line(my_head_pos_2d, ball_2d, 2, d.Color.yellow, "status", flush=False)

                # 3) 预测球位置点（你原来就有）
                d.point(slow_ball_pos, 3, d.Color.pink, "status", flush=False)
                d.point(w.ball_2d_pred_pos[-1], 5, d.Color.pink, "status", flush=True)  # 最后一笔 flush=True 刷新整组
            else:
                d.clear("status")
